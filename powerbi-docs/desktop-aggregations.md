...
ที่เก็บข้อมูลระดับตารางโดยปกติจะใช้กับคุณลักษณะการรวม สำหรับข้อมูลเพิ่มเติม โปรดดูบทความ[โหมดพื้นที่เก็บข้อมูลใน Power BI Desktop (ตัวอย่าง)](desktop-storage-mode.md)
...
### <a name="aggregation-tables-arent-addressable"></a>ไม่สามารถระบุตำแหน่งตารางการรวมได้
ผู้ใช้ที่มีการเข้าถึงชุดข้อมูลแบบอ่านอย่างเดียวไม่สามารถคิวรีตารางการรวมได้ ซึ่งเป็นการหลีกเลี่ยงข้อกังวลด้านความปลอดภัยเมื่อใช้กับ RLS ผู้บริโภคและคิวรี่อ้างอิงถึงตารางรายละเอียดไม่ใช่ตารางรวม ซึ่งไม่จำเป็นต้องรู้ว่ามีตารางรวมอยู่

ด้วยเหตุนี้ ตาราง **Sales Agg** ควรถูกซ่อนอยู่ ถ้าไม่ซ่อน กล่องโต้ตอบการจัดการการรวมจะตั้งค่าให้ซ่อนเมื่อคลิกที่ปุ่มนำไปใช้ทั้งหมด
...
* ตารางรายละเอียดต้องเป็น DirectQuery ไม่ใช่ การนำเข้า
...
### <a name="rls"></a>RLS
นิพจน์การรักษาความปลอดภัยระดับแถว (RLS) ควรกรองทั้งตารางการรวมและตารางรายละเอียดเพื่อการทำงานอย่างถูกต้อง ต่อไปนี้เป็นตัวอย่างนิพจน์ RLS บนตาราง**ภูมิศาสตร์**จะทำงานเนื่องจากภูมิศาสตร์อยู่ในด้านการกรองของความสัมพันธ์ทั้งในตาราง**ยอดขาย**และตาราง **Sales Agg** ได้ คิวรีที่ใช้งานและไม่ใช้งานตารางการรวมจะปรับใช้ RLS ได้สำเร็จ

![รวบบทบาทการจัดการต่าง ๆ](media/desktop-aggregations/manage-roles.png)

นิพจน์ RLS บนตาราง**ผลิตภัณฑ์** จะกรองเฉพาะตาราง **ยอดขาย** ไม่ใช่ตาราง **Sales Agg** ไม่แนะนำให้ดำเนินการนี้ คิวรีที่ส่งโดยผู้ใช้ที่เข้าถึงชุดข้อมูลโดยใช้บทบาทนี้จะไม่ได้รับประโยชน์จากการรวมผู้เยี่ยมชม เนื่องจากตารางการรวมเป็นการแสดงข้อมูลเดียวกันในตารางรายละเอียด ดังนั้นจึงไม่ปลอดภัยที่จะตอบคิวรีจากตารางการรวมเนื่องจากไม่สามารถใช้ตัวกรอง RLS ได้

นิพจน์ RLS บนตาราง **Sales Agg** จะกรองเฉพาะตารางการรวมเท่านั้นและไม่ใช่ตารางรายละเอียด ซึ่งไม่ได้รับอนุญาต

![รวบบทบาทการจัดการต่าง ๆ](media/desktop-aggregations/filter-agg-error.jpg)
...
กฎ RLS เดียวกันตามรายละเอียดข้างต้นสำหรับการรวมโดยอิงตามความสัมพันธ์เกี่ยวกับว่านิพจน์ RLS สามารถกรองตารางการรวม ตารางรายละเอียด หรือทั้งสองอย่าง รวมทั้งนำไปใช้กับการรวมกลุ่มตามคอลัมน์ด้วยได้หรือไม่ ในตัวอย่าง นิพจน์ RLS ที่นำไปใช้กับตาราง **Driver Activity** สามารถใช้เพื่อกรองตาราง **Driver Activity Agg** ได้เนื่องจากตารางรายละเอียดครอบคลุมทั้งกลุ่มตามคอลัมน์ในตารางการรวม อีกนัยหนึ่ง ตัวกรอง RLS บนตาราง **Driver Activity Agg** ไม่สามารถใช้กับตาราง **Driver Activity** ดังนั้นจึงไม่ได้รับอนุญาต
...
* ตาราง **Driver Activity Agg** เป็นตารางรวมระดับกลางในโหมด DirectQuery มีแถวมากกว่าหนึ่งพันล้านแถวใน Azure SQL DW และได้รับการปรับให้เหมาะกับแหล่งที่มาโดยใช้ดัชนี columnstore
...
> [!NOTE]
> ตารางการรวม DirectQuery ที่ใช้แหล่งข้อมูลที่แตกต่างกันกับตารางรายละเอียดจะได้รับการสนับสนุนเฉพาะเมื่อตารางการรวมมาจาก SQL Server, Azure SQL หรือแหล่งที่มาของ Azure SQL DW
